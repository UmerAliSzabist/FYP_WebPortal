...
export default function Home(props) {
    ...
    const [fundsType, setFundsType] = React.useState([]);
    const [getAds, setAds] = React.useState([]);

    const [currentPage, setCurrentPage] = React.useState(1);
    const [postPerPage, setPostPerPage] = React.useState(10);

    const [onClickSignIn, setOnClickSignIn] = React.useState(false);
    const [onCLickSignUp, setOnClickSignUp] = React.useState(false);
    const [onCLickCompare, setOnClickCompare] = React.useState(false);

    const [allAMC, setAllAMC] = React.useState('');

    const [selectedAmc, setSelectedAmc] = React.useState(0);
    ...
    const currentPosts = getAds.slice(indexOfFirstPost, indexOfLastPost)

    const paginate = (pageNumber) => setCurrentPage(pageNumber)

    const handleSignIn = () => {
        setOnClickSignIn(true);
    }

    const handleSignUp = () => {
        setOnClickSignUp(true);
    }

    const handleCompare = () => {
        setOnClickCompare(true);
    }

    if (onClickSignIn === true) {
        return <Redirect to='/login' />
    }

    if (onCLickSignUp === true) {
        return <Redirect to='/Signup' />
    }

    if (onCLickCompare === true) {
                                        ...
                                        (
                                            <div>
                                                        ...
                                                        <div key={fund._id} style={{
                                                            ...
                                                            <div style={{ display: 'flex', flex: 0.2, margin: 5, flexDirection: 'column', textAlign: 'center', justifyContent: 'center', alignContent: 'center', alignItems: 'center' }}>
                                                                <InvestButton variant="outlined" style={{ width: '100px', textTransform: "none", }}>
                                                                    <Link to={{ pathname: '/adsApprovalDetails', state: { fund } }} style={{ textDecoration: 'none', textTransform: "none" }}>
                                                                        Details
                                                                    </Link>
                                                                </InvestButton>
                                                            </div>
                                                ...
                                                <div style={{ flex: 0.2, justifyContent: 'center', alignContent: 'center', alignItems: 'center', textAlign: 'center' }}>
                                                    <Pagination postPerPage={postPerPage} totalPost={getAds.length} paginate={paginate} />
                                                </div>
                                            </div>
                                        ) : <div style={{ display: 'flex', flex: 0.5, margin: 5, flexDirection: 'column', textAlign: 'center', justifyContent: 'center', alignContent: 'center', alignItems: 'center' }}>
